...
def find_valid_directions(grid, player, position) :
    ...
    for direction in DIRECTIONS:
        dy, dx = direction
        j, i = y+dy, x+dx
        opponent_player_found, player_found = False, False
        while 0<=j<8 and 0<=i<8 and grid[j,i]!=EMPTY and not player_found :
            if not opponent_player_found and grid[j,i]==opponent_player :
                opponent_player_found = True
            elif not player_found and grid[j,i]==player :
                player_found = True
            j, i = j+dy, i+dx
        if player_found:
            ...
            if event.type == pygame.QUIT :
                ...
                if event.button == 1: # click sur bouton gauche
                    ...
                    if (y,x) != (-1,-1) and grid[y,x]==EMPTY:
                        ...
                        if number_of_tokens != (NUMBER_OF_BLACK_TOKEN, NUMBER_OF_WHITE_TOKEN):
                            NUMBER_OF_BLACK_TOKEN, NUMBER_OF_WHITE_TOKEN = number_of_tokens
